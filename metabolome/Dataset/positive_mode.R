...
refmet_pos <- read.delim("C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Documents/Projects/BioMe/metabolome/RefMet_C18-HILICpos_Targets_7.5ppm_10percent-Studies_14May21.txt")

min(refmet_pos$Max_number_studies_reported) # All metabolites were reported in at least 14 
...
refmet_pos <- refmet_pos[which(y>0.75),] 
...
chunk_18 <- apply(t_refmet_pos[,!(colnames(t_refmet_pos) %in% c("SAMPLEID"))], 2, function(x) scale(log(x + 1, base = 2)))
...
data_pred <- data_all__pos[,colnames(data_all__pos) %in% c("SAMPLEID","DID","PRS_score","age_at_enrollment","self_reported_race",
...
write.table(refmet_pos, "~/Projects/BioMe/proteome/input/analysis_sample/met/BioMe_metabolome_pos_PFAS_annotation.txt", row.names = FALSE)
write.table(metabolome_restrict, "~/Projects/BioMe/proteome/input/analysis_sample/met/BioMe_metabolome_pos_PFAS_wide.txt", row.names = FALSE)
