...
##------------------------------------------- import data
BioMe_proteome_PFAS_wide <- fread("/sc/arion/work/yaom03/biome_proteome/pfas_metabolome/bwqs/BioMe_metabolome_pos_PFAS_wide.txt")

# /sc/arion/work/yaom03/biome_proteome/pfas_metabolome/bwqs
##------------------------------------------- prepare data

bwqs_data<- BioMe_proteome_PFAS_wide %>% 
  dplyr::select(starts_with("pos"), ends_with("_q"), self_reported_race, gender, age_at_enrollment, c_date_enrl, sw)
...
protein<- colnames(bwqs_data %>% dplyr::select(starts_with("pos")))
...
write.table(bwqs_pfas_met_model, "/sc/arion/work/yaom03/biome_proteome/pfas_metabolome/bwqs/met_vs_pfas_bwqs_pos.txt", row.names = FALSE)
write.table(bwqs_pfas_weight, "/sc/arion/work/yaom03/biome_proteome/pfas_metabolome/bwqs/met_pfas_weight_pos.txt", row.names = FALSE)
